#!/usr/bin/env python3
"""
Smoke test: extract markdown/text from a PDF using Docling (same code as the app).

Requires ``docling`` from requirements.txt. Run from repo root:

    python scripts/test_docling_pdf.py /path/to/document.pdf
    python scripts/test_docling_pdf.py ./some.pdf --max-chars 500
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Extract markdown/text from a PDF via Docling."
    )
    parser.add_argument(
        "pdf",
        type=Path,
    ...
    try:
        text = extract_text_with_docling(str(pdf))
